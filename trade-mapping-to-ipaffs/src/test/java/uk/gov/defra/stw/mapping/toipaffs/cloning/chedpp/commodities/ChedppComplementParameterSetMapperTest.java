...
import uk.gov.defra.stw.mapping.toipaffs.common.chedpp.commodities.ChedppControlledAtmosphereContainerMapper;
import uk.gov.defra.stw.mapping.toipaffs.common.chedpp.commodities.ChedppPackageTypeMapper;
import uk.gov.defra.stw.mapping.toipaffs.common.chedpp.commodities.ClassMapper;
import uk.gov.defra.stw.mapping.toipaffs.common.chedpp.commodities.FinishedOrPropagatedMapper;
import uk.gov.defra.stw.mapping.toipaffs.common.chedpp.commodities.VarietyMapper;
...
class ChedppComplementParameterSetMapperTest {
  ...
  void setup() throws JsonProcessingException {
    ...
    mapper = new ChedppComplementParameterSetMapper(
        ...
        new ChedppNetWeightMeasureMapper(new NetWeightMeasureKeyDataMapper(), chedppQuantityTypeMapper),
  ...
  void map_ReturnsComplementParameterSet_WhenComplete() throws JsonProcessingException {
    ...
    String expectedComplementParameterSet = ResourceUtils
        .readFileToString("classpath:cloning/chedpp/partone/commodities/chedpp_ipaffs_complementParameterSet_complete.json");
    assertThat(complementParameterSet.get(0).getKeyDataPair()).hasSize(9);
    assertThat(actualComplementParameterSet).isEqualTo(expectedComplementParameterSet);
  }

  @Test
  void map_ReturnsComplementParameterSetWithoutTypeQuantity_WhenQuantityTypeMapperReturnsNull() {
    IncludedSpsTradeLineItem includedSpsTradeLineItem =
        findByUnitCode(spsCertificate.getSpsConsignment().getIncludedSpsConsignmentItem().get(0).getIncludedSpsTradeLineItem(), "KGM");

    includedSpsTradeLineItem.getNetWeightMeasure().setUnitCode("Unknown");

    List<ComplementParameterSet> complementParameterSet = mapper.map(spsCertificate);

    assertThat(findByKey(complementParameterSet.get(0).getKeyDataPair(), TYPE_QUANTITY)).isNull();
  }

  @Test
  void map_ReturnsComplementParameterSetWithoutTypeQuantityValue_WhenQuantityTypeMapperReturnsPieces() {
    IncludedSpsTradeLineItem includedSpsTradeLineItem =
        findByUnitCode(spsCertificate.getSpsConsignment().getIncludedSpsConsignmentItem().get(0).getIncludedSpsTradeLineItem(), "KGM");

    includedSpsTradeLineItem.getNetWeightMeasure().setUnitCode("H87");

    List<ComplementParameterSet> complementParameterSet = mapper.map(spsCertificate);

    ComplementParameterSetKeyDataPair keyDataPair = findByKey(complementParameterSet.get(0).getKeyDataPair(), TYPE_QUANTITY);
    assertThat(keyDataPair.getData()).isEqualTo("Pieces");
    assertThat(findByKey(complementParameterSet.get(0).getKeyDataPair(), "netweight")).isNull();
  }

  @Test
  void map_ReturnsComplementParameterSetWithoutQuantity_WhenQuantityMapperReturnsNull() {
    IncludedSpsTradeLineItem includedSpsTradeLineItem =
        findByUnitCode(spsCertificate.getSpsConsignment().getIncludedSpsConsignmentItem().get(0).getIncludedSpsTradeLineItem(), "KGM");

    includedSpsTradeLineItem.setNetWeightMeasure(null);

    List<ComplementParameterSet> complementParameterSet = mapper.map(spsCertificate);

    assertThat(findByKey(complementParameterSet.get(0).getKeyDataPair(), QUANTITY)).isNull();
  }
  ...
  private IncludedSpsTradeLineItem findByUnitCode(List<IncludedSpsTradeLineItem> items, String unitCode) {
    return items.stream()
        .filter(item -> item.getNetWeightMeasure().getUnitCode().equals(unitCode))
        .findFirst()
        .orElse(null);
  }
